...
class ImageEncoder(ResNet):
	def  __init__(self, size=[512, 512], resSet=[2, 2, 2, 2], gar_latent_size=256):
		...
		self.pose_fc = nn.Linear(2048, 24 * 3)  # output matrix formation
		self.tran_fc = nn.Linear(2048, 3)
		self.rotate_fc = nn.Linear(2048, 3 * 3)
		self.scale_fc = nn.Linear(2048, 1)
		self.tran_dp = nn.Dropout(p=0.3)
		self.rotate_dp = nn.Dropout(p=0.3)
		self.scale_dp = nn.Dropout(p=0.3)
...
class ImageReconstructModel(Module):
	...
	def filter(self, imgs, segimg, lines, input_imgbatch=None, **kwargs):
		gtypes = self.gtypes
	...
	def repose(self, imgs, weight, cam_k=None, **kwargs):
		# gtypes = self.gtypes
		# batch_num = self.batch_num
		...
		if result.shape[0]==ws.shape[0]:
			result = result.to(ws.device)
			ws = result
		if cam_k is None:
			cam_k = torch.Tensor([[3.0375e+03, 0.0000e+00, 2.7000e+02], [0.0000e+00, 3.0375e+03, 2.7000e+02],
		...
		displacement_datas = []
		for ind, (pca_params, pca_ps), (shapes_gtype, _, latents_gtype), (
		deform_ps, deform_ns, pro_fs, Rs_gtype, Ts_gtype) in zip(ordered_gtypes2, pca_datas, ordered_datas, ordered_datas2):
	...
	def forward(self, image, segImage, label, labels_lines, alpha_labels, weight_labels, gtypes=None):
		...
		self.filter(image, segImage, labels_lines)

		# Phase 2: repose
		self.repose(image, weight_labels)
