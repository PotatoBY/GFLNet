...
def getObj(bps, face_index, type):
    batch_vertices = bps
    batch_faces = face_index
    up_faces = batch_faces[:garmentfnums[index[0]]]
    bottom_faces = batch_faces[garmentfnums[index[0]]:garmentfnums[index[0]] + garmentfnums[index[1]]]
    tris = [up_faces, bottom_faces]

    if types == 0:
        up_vertices = batch_vertices[:garmentvnums[index[0]]]
        bottom_vertices = batch_vertices[garmentvnums[index[0]]:garmentvnums[index[0]] + garmentvnums[index[1]]]
...
def compute_loss(opt, res, labels):

    # init.
    # chamfer_dis, gtV_2_estM_dis, estV_2_gtM_dis, estMesh, gtMesh = None, None, None, None, None
    chamfer_dist = ChamferDistance()
    estV_2_gtM_dis = 0
    chamfer_dis = 0
    # index = res['gtypes'].nonzero(as_tuple=False)[:, 1]
    for ind in range(opt.batch_size):
        ...
        label_displacements = np.concatenate((label_displacements, labels['up'][ind]), axis=0)
        label_displacements = np.concatenate((label_displacements, labels['bottom'][ind]), axis=0)
        gtMesh = np.delete(label_displacements, 0, axis=0)

        # compute gt vertex to est mesh distance
        cuda = torch.device('cuda') if len(opt.gpu_ids) > 1 else torch.device('cuda:%d' % opt.gpu_id)
        estMesh_v = estMesh.contiguous()  # e.g. (46918, 3), torch.float32
        gtMesh_v = torch.from_numpy((gtMesh[:, :]).astype(np.float32)).to(cuda).contiguous()  # e.g. (93182, 3), torch.float32
    ...
    # return
    return chamfer_dis/opt.batch_size, estV_2_gtM_dis/opt.batch_size
...
def calc_error_gfl(opt, net, cuda, dataset, num_tests):
    """
    return
        avg. {error, IoU, precision, recall} computed among num_test frames, each frame has e.g. 5000 query points for evaluation.
    """
    ...
    with torch.no_grad():
        ...
        with tqdm(dataset, unit="batch") as tepoch:
            test_idx = 0
            for data in tepoch:  # 14496 iters for each epoch
                # get a training batch
                if test_idx > num_tests:
                    break
                image_tensor = data['img'].to(device=cuda)  # (B==2, C, W, H) RGB, 3x512x512 images, float -1. ~ 1., bg is all ZEROS not -1.
                seg_tensor = data['seg'].to(device=cuda)  # (B==2, C, W, H) RGB, 3x512x512 images, float -1. ~ 1., bg is all ZEROS not -1.
                label_tensor = data['labels']
                labels_lines_tensor = data['label_lines']
                alpha_labels_tensor = data['alpha_label']
                img_gtypes = data['img_gtypes']

                # network forward pass
                res, error = net.forward(image_tensor[0], seg_tensor[0], label_tensor, labels_lines_tensor, alpha_labels_tensor, gtypes=img_gtypes)  # get output for every net

                if len(opt.gpu_ids) > 1: error = error.mean()

                # compute loss
                cd, psd = compute_loss(opt, res, label_tensor[0]) # R, R, R

                cd_arr.append(cd)
                psd_arr.append(psd)
                loss_arr.append(error.item())
                test_idx += 1

    return np.average(cd_arr), np.average(psd_arr), np.average(loss_arr)
