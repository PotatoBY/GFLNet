...
class TrainDatasetGFL(Dataset):
    ...
    def read_img(self, file, mask_path):
        ...
        # else:
        #     nimg = img
        # nimg = cv2.resize(nimg, (512, 512))
        # nimg = nimg.transpose(2, 0, 1)
        # nimg = nimg.astype(np.float32) / 255.
        return image_padded

    def get_render_list(self, args):
        '''
        Return the render data
        :return:
            'img': [C, W, H] for 3x512x512 images
            'seg': [C, W, H] for 3x512x512 segs
            'mask': [1, W, H] for 1x512x512 masks
        '''
        # init.
        train_num = self.totalsize
        alpha_label_files = []
        weight_label_files = []
        img_files = []
        seg_files = []
        ...
        datapath = '%s/imgfiles.txt' % (args.datasetDir)
        label_dir = '%s/all_train_datas/' % (args.datasetDir)
        label_lines_dir = '%s/label_datas/' % (args.datasetDir)

        with open(datapath, "r") as f:
            for line in f.readlines():
                img_dir = args.datasetDir+ "/" + line.strip('\n')
                seg_dir = img_dir.replace("all_train_render_datas", "seg_edg/cihp_parsing_maps")
                seg_dir = seg_dir.replace(".jpg", "_vis.png")
                name = img_dir.split('/')[-1].split('.')[0]
                edge_dir = img_dir.replace(name + ".jpg", 'fg' + name + ".png")
                seg_files.append(seg_dir)
                edge_files.append(edge_dir)
                img_files.append(img_dir)
                alpha_dir = img_dir.replace("short_", "short-")
                alpha_dir = alpha_dir.split('_')
                up_alpha_dir = '../neutral_datas/' + alpha_dir[5] + '/' + alpha_dir[4].split('/')[1] + '/' + alpha_dir[6] + '/pca_param.npy'
                bottom_alpha_dir = '../neutral_datas/' + alpha_dir[7] + '/' + alpha_dir[4].split('/')[1] + '/' + alpha_dir[8] + '/pca_param.npy'
                up_alpha_dir = up_alpha_dir.replace("short-", "short_")
                bottom_alpha_dir = bottom_alpha_dir.replace("short-", "short_")
                up_weight_dir = up_alpha_dir.replace("pca_param.npy", "garment_pca_weights.npy")
                bottom_weight_dir = bottom_alpha_dir.replace("pca_param.npy", "garment_pca_weights.npy")
                alpha_label_files.append([up_alpha_dir, bottom_alpha_dir])
                weight_label_files.append([up_weight_dir, bottom_weight_dir])
        ...
        label_files = []
        label_files.extend(glob(osp.join(label_dir, '*.npz')))
        label_files = sorted(label_files, key=lambda name: int(name.split('/')[-1][:-4]))
        label_lines_files = []
        label_lines_files.extend(glob(osp.join(label_lines_dir, '*.txt')))
        label_lines_files = sorted(label_lines_files, key=lambda name: int(name.split('/')[-1][:-4]))
        ...
        for index in range(len(img_files)):
            if img_num >= train_num:
                break
            if os.path.exists(img_files[index]):
                img_file.append(img_files[index])
                seg_file.append(seg_files[index])
                edge_file.append(edge_files[index])
                label_file.append(label_files[index])
                label_lines_file.append(label_lines_files[index])
                alpha_label_file.append(alpha_label_files[index])
                weight_label_file.append(weight_label_files[index])
                img_num += 1

        self.img_file = img_file
        self.seg_file = seg_file
        self.mask_file = edge_file
        self.label_file = label_file
        self.label_lines_file = label_lines_file
        self.alpha_label_file = alpha_label_file
        self.weight_label_file = weight_label_file
        self.img_gtypes = img_gtypes
    ...
    def get_item(self, index):
        ...
        if not self.is_train: index += len(self.training_inds)
        imgs = []
        imgs.append(self.read_img(self.img_file[index],self.mask_file[index]))
        img_paths = []
        img_paths.append(self.img_file[index])
        segs = []
        segs.append(self.read_img(self.seg_file[index],self.mask_file[index]))
        labels = []
        labels.append(np.load(self.label_file[index]))

        label_lines = []
        label_lines.append(self.read_txt(self.label_lines_file[index]))
        alpha_labels = []
        alpha_labels.append([np.load(self.opt.datasetDir + self.alpha_label_file[index][0].split('..')[1]), np.load(self.opt.datasetDir + self.alpha_label_file[index][1].split('..')[1])])

        weight_labels = []
        weight_labels.append([np.load(self.opt.datasetDir + self.weight_label_file[index][0].split('..')[1]),np.load(self.opt.datasetDir + self.weight_label_file[index][1].split('..')[1])])

        img_gtypes = []
        img_gtypes.append(self.img_gtypes[index])
        render_data = {'img': torch.stack(imgs, dim=0),
                'img_paths': img_paths,
                'seg': torch.stack(segs, dim=0),
                'labels': labels,
                'label_lines': label_lines,
                'alpha_label': alpha_labels,
                'weight_label': weight_labels,
                'img_gtypes': img_gtypes
                }
        # res.update(render_data)
        ...
        #     """
        #     # sample_data = self.select_sampling_method_iccv_online(dataConfig) if self.opt.online_sampling else self.select_sampling_method_iccv_offline()
        #     sample_data = self.select_sampling_method_iccv_offline(index) if (
        #                 self.is_train and (not self.opt.online_sampling)) else self.select_sampling_method_iccv_online(
        #         dataConfig)
        #     res.update(sample_data)
        #
        #     # check "calib" by projecting "samplings" onto "img"
        ...
        #         print("visualCheck_0: see if 'samples' can be properly projected to the 'img' by 'calib'...")
        #
        #         img, segimg, edgeimg = self.read_img(res['img']), self.read_img(res['seg']), self.read_img(res['mask'])
        #         # image RGB
        #         img = np.uint8((np.transpose(img[0].numpy(), (1, 2, 0)) * 0.5 + 0.5)[:, :, ::-1] * 255.0)  # HWC, BGR, float 0 ~ 255, de-normalized by mean 0.5 and std 0.5
        #         cv2.imwrite("./sample_images/%06d_img.png" % (index), img)
        #
        #         segimg = np.uint8((np.transpose(segimg[0].numpy(), (1, 2, 0)) * 0.5 + 0.5)[:, :, ::-1] * 255.0)  # HWC, BGR, float 0 ~ 255, de-normalized by mean 0.5 and std 0.5
        #         cv2.imwrite("./sample_images/%06d_segimg.png" % (index), segimg)
        #
        #         # mask
        #         mask = np.uint8(edgeimg[0, 0].numpy() * 255.0)  # (512, 512), 255 inside, 0 outside
        #         cv2.imwrite("./sample_images/%06d_mask.png" % (index), mask)
        #
        #         # orthographic projection
        #         rot = res['calib'][0, :3, :3]  # R_norm(-1,1)Cam_model, assuming that ortho. proj.: cam_f==256, c_x==c_y==256, img.shape==(512,512,3)
        ...
        #         if True:
        #             # img
        #             img = np.uint8((np.transpose(res['img'][0].numpy(), (1, 2, 0)) * 0.5 + 0.5)[:, :,
        ...
        #             meshVoxels_check = res["meshVoxels"][0].numpy()  # DHW
        #             meshVoxels_check = np.transpose(meshVoxels_check, (2, 1, 0))  # WHD, XYZ
        #             save_volume(meshVoxels_check, fname="./sample_images/%06d_meshVoxels_input.obj" % (index),
        #                         dim_h=consts.dim_h, dim_w=consts.dim_w, voxel_size=consts.voxel_size)
        #
        #         # ----- target view, a random view from {front, right, back, left} -----
        #         if self.opt.use_view_pred_loss:
        #             # init.
        ...
        #
        #             # target img
        #             img = np.uint8((np.transpose(res['target_view'].numpy(), (1, 2, 0)) * 0.5 + 0.5)[:, :,
        #                            ::-1] * 255.0)  # HWC, BGR, float 0 ~ 255, de-normalized by mean 0.5 and std 0.5
        #             cv2.imwrite("./sample_images/%06d_img_target_%s.png" % (index, target_view_name), img)
        #
        #             # target mesh voxels (this is to verify voxel transformation functions)
        #             meshVoxels_target = self.transform_voxels_to_target(voxels_CDHW=res["meshVoxels"],
        #                                                                 target_view_idx=res["view_directions"])
        #             meshVoxels_target = np.transpose(meshVoxels_target[0].numpy(), (2, 1, 0))  # WHD, XYZ
        #             save_volume(meshVoxels_target,
        #                         fname="./sample_images/%06d_meshVoxels_target_%s.obj" % (index, target_view_name),
        #                         dim_h=consts.dim_h, dim_w=consts.dim_w, voxel_size=consts.voxel_size)
        #
        #         pdb.set_trace()
